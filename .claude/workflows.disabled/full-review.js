...
const DIMENSIONS = [
  {
    key: 'architecture',
    prompt: `Đọc src/ của nodejs-template-service. Tìm vi phạm kiến trúc phân lớp:
...
import tương đối thay vì subpath alias #..., handler async thiếu asyncHandler.`,
  ...
  {
    key: 'security',
    prompt: `Đọc src/ của nodejs-template-service. Tìm rủi ro bảo mật: SQL nối chuỗi trong repository,
...
jwt.decode dùng thay cho jwt.verify.`,
  ...
  {
    key: 'correctness',
    prompt: `Đọc src/ của nodejs-template-service. Tìm lỗi đúng đắn: promise không await, db.one dùng ở
...
signature errorHandler của Express (phải đủ 4 tham số mới được nhận là error middleware).`,
  ...
  {
    key: 'consistency',
    prompt: `Đọc src/ của nodejs-template-service. Tìm điểm không nhất quán: tên file lệch quy ước
